class TimeMap:
    ...
    def get(self, key: str, timestamp: int) -> str:
        ...
        if found == -1: #iterate the pointer back until it finds the appropiate value
            while mid > 0 and inner.timestamps[mid] >= timestamp:
                mid -= 1
            #print(f"after iterating: {inner.val_map[inner.timestamps[mid]]}")
            t = inner.timestamps[mid]
            if t <= timestamp:
                return inner.val_map[inner.timestamps[mid]]
            else:
